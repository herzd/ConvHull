* convex hull algorithm in Python
  In this file I document my work on the 'exact' (no-float) version of a convex hull algorithm implemented in the programming language Python 3. Changes on the initial files (~main.py~, and ~chm_exact.py~) will be produced from this file. These will be called ~main_v2.py~ and ~chm_exact_v2.py~. The main goal of this work is to find ways to speed up the calculation by means of parallelization where possible.
...
*** definitions
    + convex set of points :: set of points in an Euclidean space, that contains the line segments connecting each pair of its points.
    + convex hull :: For a given set X: the minimal convex set containing X/the intersection of all convex sets containing X/the set of all convex combinations of points in X/the union of all simplices with vertices in X
**** main program v2
#+NAME: main_v2.py
...
# additional functions for future implementation of the quickhull algorithm
def get_max_x(POINTS):
    '''takes a set of points represented as tuples and returns a tuple of the
    point with maximal x-value. In case of multiple points, the first one
    of an intermediate list is selected.'''
    MAX_POINTS = []
    X_MAX_VALUE = max([TUPLE[0] for TUPLE in POINTS])
    [MAX_POINTS.append(TUPLE) for TUPLE in POINTS if TUPLE[0]==X_MAX_VALUE]
    MAX_POINTS = MAX_POINTS[0]
    MAX_POINT = tuple(MAX_POINTS)
    return MAX_POINT

def get_min_x(POINTS):
    '''takes a set of points represented as tuples and returns a tuple of the
    point with minimal x-value. In case of multiple points, the first one
    of an intermediate list is selected.'''
    MIN_POINTS = []
    X_MIN_VALUE = min([TUPLE[0] for TUPLE in POINTS])
    [MIN_POINTS.append(TUPLE) for TUPLE in POINTS if TUPLE[0]==X_MIN_VALUE]
    MIN_POINTS = MIN_POINTS[0]
    MIN_POINT = tuple(MIN_POINTS)
    return MIN_POINT

    

def remove_points_from_inspection(POINTS):
    '''remove points with maximum and minimum x-values from list.'''
    MAX_X_POINT = get_max_x(POINTS)
    MIN_X_POINT = get_min_x(POINTS)
...
# leave this in for now (but not in use in original version either)
# if __name__ == "__main__":
#     filename = sys.argv[1]
#     dims = sys.argv[2]
#     compute_CH(filename, map(int, dims.split(",")))
